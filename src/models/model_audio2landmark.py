...
class Audio2landmark_content(nn.Module):
    ...
    def __init__(self, num_window_frames=18, in_size=80, lstm_size=AUDIO_FEAT_SIZE, use_prior_net=False,
                 hidden_size=256, num_layers=3, drop_out=0, bidirectional=False):
        ...
        if (use_prior_net):
            self.bilstm = nn.LSTM(input_size=lstm_size,  # 161
                                  hidden_size=hidden_size,  # 256
                                  num_layers=num_layers,  # 3
                                  dropout=drop_out,  # 0.5
...
class Audio2landmark_pos(nn.Module):
    ...
    def forward(self, au, emb, face_id, fls, z, add_z_spk=False, another_emb=None):
        ...
        audio_encode, (_, _) = self.audio_content_encoder(au)  # LSTM
        ...
        if add_z_spk:
            ...
            z_spk = torch.tensor(torch.randn(spk_encode.shape) * 0.01, requires_grad=False, dtype=torch.float).to(
                device)
