...
class APC_encoder(nn.Module):
    ...
    def forward(self, inputs, lengths):
        ...
        with torch.no_grad():
            ...
            # inputs.shape = torch.Size([1, 1374, 80])

            seq_len = inputs.size(1)
            packed_rnn_inputs = pack_padded_sequence(inputs, lengths, True)
        
            for i, layer in enumerate(self.rnns):
                # packed_rnn_outputs = torch.Size([1374, 512])
                packed_rnn_outputs, _ = layer(packed_rnn_inputs)  # GRU(80, 512, batch_first=True)
                # rnn_outputs = torch.Size([1, 1374, 512])
                ...
                if i + 1 < len(self.rnns):
                    # 使用pad_packed_sequence把压紧的序列再填充回来, 得到与rnn_outputs相同的格式, 再进行后续的残差叠加
...
class WaveNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.input_channels = 204  # 先直接改成204
...
#        self.output_channels = (2 * self.ndim + 1) * self.ncenter
    ...
    def forward(self, input, cond=None):
        ...
        # input.shape = torch.Size([1, 12, 255]), cond.shape = torch.Size([1, 512, 255])
...
class residual_block(nn.Module):
    ...
    def __init__(self,
        ...
        self.bias = use_bias
        self.cond = cond
        self.cond_channels = cond_channels  # 改成256
    ...
    def forward(self, input, cond=None):
        ...
        # residual and skip
        # 这里加上pre_head_pose, self.residual_conv(x)就是一个残差(offset) + pre_head_pose,
